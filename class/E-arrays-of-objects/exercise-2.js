...
and a list of transportations available to go there.

1) Filter the travelDestinations array to return all destination names
reachable within 500 kms.
2) Find a destination name reachable by ferry.
3) Print in the console all the destination names more than 300 kms
far away and reachable by train.
*/
...
let destinationNamesWithin500Kms = travelDestinations
  .filter(function (td) {
    return td.distanceKms < 500;
  })
  .map(function (td) {
    return td.destinationName;
  });

let destinationNameReachableByFerry = travelDestinations.find(function (td) {
  return td.transportations.includes("ferry");
}).destinationName;

let destinationNamesMoreThan300KmsAwayByTrain = travelDestinations
  .filter(function (td) {
    return td.distanceKms > 300 && td.transportations.includes("train");
  })
  .map(function (td) {
    return td.destinationName;
  });
